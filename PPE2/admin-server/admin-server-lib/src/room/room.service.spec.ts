...
let roomService, dataAccess, ressourceAccess, room, image, logger;
...
describe('Room Service', () => {
  beforeEach(() => {
    ...
    roomService = new RoomService({ dataAccess, ressourceAccess, logger });
  ...
  describe('modifyRoom', () => {
    ...
    it('Should call the addRoomImage method of the ressource access if the field is filled and the format is validated', done => {
      ressourceAccess.addRoomImage = () => Promise.resolve(done());
      room.image = image;
  ...
  describe('getRoomImage', () => {
    ...
    it('Should call the getRoomImage method of the ressource access', done => {
      ressourceAccess.getRoomImage = () => Promise.resolve(done());

      roomService.getRoomImage(1);
    });

    it('Should reject the Promise if the ressource access rejected the Promise', done => {
      ressourceAccess.getRoomImage = () => Promise.reject('ERR');

      roomService.getRoomImage(1)
        .catch(() => done());
    });

    it('Should resolve with the image if the ressource access resolved the Promise', done => {
      ressourceAccess.getRoomImage = () => Promise.resolve(image);
